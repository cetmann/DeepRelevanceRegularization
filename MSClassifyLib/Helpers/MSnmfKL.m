...
%(mass spectrometry)
param.lambda    = 0.1*10^(-1); %sparsity of spectra
...
while  t<numberit  
    ...
    l1normsX = repmat(sum(abs(X),2),1,size(X,2));
    l1normsXdimK = repmat(transpose(sum(abs(X),2)),size(K,1),1);
    X = X./l1normsX;
    K = K.*l1normsXdimK;
