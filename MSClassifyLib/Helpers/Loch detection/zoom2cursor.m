...
% FUNCTION ZOOM2CURSOR
%
% zoom2cursor, without arguments, will activate the current axis and automatically zoom the image to the
...
zoomparams.pct = 1; %Default value
...
%setappdata(currfig, 'bdfcnhandle', @bdfcn);
setappdata(gcf, 'bdfcnhandle',@bdfcn);

set(currfig,'doublebuffer','on','windowbuttonmotionfcn','feval(getappdata(gcf,''zoomfcnhandle''));');
%endbutton = uicontrol('style','pushbutton','string','X','units','normalized','position',[0 0 0.04 0.065],'foregroundcolor','y','backgroundcolor','k',...
