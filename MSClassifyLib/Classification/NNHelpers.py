# -*- coding: utf-8 -*-
"""
Created on Tue Feb 14 09:02:47 2017
...
from lasagne.layers import Conv2DLayer, \
    NonlinearityLayer, DenseLayer, BatchNormLayer,  \
    ElemwiseSumLayer, batch_norm
from lasagne.nonlinearities import linear, softmax
from lasagne.init import HeUniform as He
...
def ResidualLayer(inputLayer,num_filters,filter_size=(5,1),stride=(1,1),nonlinearity=rectify):
    ...
    layer = Conv2DLayer(layer,num_filters,filter_size,stride=(1,1),nonlinearity=linear,pad='same',W=He(),b=None)
    layer = BatchNormLayer(layer)
    layer = NonlinearityLayer(layer,nonlinearity=nonlinearity)
    layer = Conv2DLayer(layer,num_filters,filter_size,stride=stride,nonlinearity=linear,pad='same',W=He(),b=None)
    if (num_filters != inputLayer.output_shape[1]) or (stride!=(1,1)):
        inputLayer = Conv2DLayer(inputLayer,num_filters,filter_size=(1,1),stride=stride,nonlinearity=linear,pad='same',W=He(),b=None)
    layer = ElemwiseSumLayer([inputLayer,layer])
    return layer     
    
def ResidualLayerBottleneck(inputLayer,num_filters,filter_size=(5,1),stride=(1,1),nonlinearity=rectify):
    layer = inputLayer
    layer = NonlinearityLayer(layer,nonlinearity=nonlinearity)
    layer = Conv2DLayer(layer,num_filters,filter_size,stride=(1,1),nonlinearity=linear,pad='same',W=He(),b=None)
    layer = BatchNormLayer(layer)
